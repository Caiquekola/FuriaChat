...
const Chat: React.FC = () => {
  ...
  // Quando o usuário logar, exibir mensagem de boas-vindas
  useEffect(() => {
    if (currentUser) {
      const welcomeMessage: Message = {
        id: `welcome-${currentUser.id}`,
        content: `👋 Bem-vindo, ${currentUser.username}! Agora você pode conversar.`,
        sender: {
          id: 'system',
          username: 'Sistema',
          avatar: 'https://i.imgur.com/8Km9tLL.png', // Avatar genérico de sistema
          isAdmin: true
        },
        timestamp: new Date(),
        reactions: []
      ...
      // Verifica se já adicionou a mensagem para não duplicar
      setMessages(prev => {
        if (prev.some(m => m.id === welcomeMessage.id)) return prev;
